...
async function main() {
  ...
  /* The numeric age field and its 2/10/30/50/70 chips were replaced by five
     age bands. A crew at a roadside can always say which band a patient is in
     and often cannot say more; the band still resolves to the single number
     the backend, triage and the ER board already expect, so the data contract
     is untouched. These checks are the replacement for the old one and are
     deliberately stricter: they pin the bands, the mapping, and the fact that
     nothing numeric is left to type. */
  check("age is chosen from five bands, not typed", function () {
    var bands = $("ageChips").querySelectorAll("[data-age-band]");
    assert.strictEqual(bands.length, 5);
    var ids = Array.prototype.map.call(bands, function (b) { return b.dataset.ageBand; });
    assert.deepStrictEqual(ids, ["baby", "child", "adult", "senior", "unknown"]);
  });
  check("no free numeric age input is offered any more", function () {
    var age = $("age");
    assert.strictEqual(age.type, "hidden", "the age field is still typeable");
    assert.strictEqual($("ageChips").querySelectorAll("input").length, 0);
  });
  check("choosing a band sets the value the backend already expects", function () {
    $("ageChips").querySelector("[data-age-band='adult']").click();
    var stored = $("age").value;
    assert.notStrictEqual(stored, "", "nothing was stored for Adult");
    assert.ok(Number.isFinite(Number(stored)), "a band must resolve to a number: " + stored);
    /* The value must land back in the band it came from, or the ER board
       would show a different age group from the one the crew tapped. */
    assert.strictEqual(w.GH.describeAgeBand(stored).id, "adult");
    assert.strictEqual(w.__GH.state().selected.ageBand, "adult");
  });
  check("the selected band is the only one marked, for sighted and screen-reader users alike", function () {
    var on = $("ageChips").querySelectorAll(".is-on");
    assert.strictEqual(on.length, 1);
    assert.strictEqual(on[0].dataset.ageBand, "adult");
    assert.strictEqual(on[0].getAttribute("aria-checked"), "true");
    assert.strictEqual($("ageChips").querySelector("[data-age-band='baby']").getAttribute("aria-checked"), "false");
  });
  check("Unknown is a real answer and clears the value rather than guessing one", function () {
    $("ageChips").querySelector("[data-age-band='unknown']").click();
  ...
  check("age and sex are marked required, and neither blocks a broadcast", function () {
    ...
    /* Required means "the form insists on an answer", not "the form refuses
       to send". A demographic field must never stand between a crew and a
       hospital, so Unknown is always one tap away and never a blocker. */
    assert.strictEqual($("submitHint").className.indexOf("is-bad"), -1);
  });
  ...
  check("selections reset (consciousness, sex, age band, blood group, FAST)", function () {
    var s = w.__GH.state();
    ...
    assert.strictEqual(s.selected.ageBand, "unknown");
    /* Reset must also forget that a person answered, so the next case asks
       again rather than carrying the last patient's age group forward. */
    sameShape(s.touched, { age: false, gender: false });
