...
/** "58 M" — enough for a handover, not enough to identify anybody. */
export function describePatient(entry) {
  if (!entry) return '—';
  const bits = [];
  if (entry.age !== null && entry.age !== undefined && entry.age !== '') bits.push(String(entry.age));
