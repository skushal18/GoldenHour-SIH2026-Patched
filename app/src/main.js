...
import { createCaseBook, describePatient, describeWhen, STATUS_TEXT, STATUS_TONE } from './modules/cases.js';
...
function boot(win, doc) {
  ...
  function applyTheme(value) {
    ...
    try { storage.setItem('goldenhour.theme', JSON.stringify(v)); } catch (_) {}
  ...
  function currentTheme() {
    try { return JSON.parse(storage.getItem('goldenhour.theme') || '"auto"'); } catch (_) { return 'auto'; }
  }
  ...
  function showView(name) {
    ...
    if (name === 'settings') {
      const more = $('moreCard');
      if (more) { more.open = true; more.scrollIntoView && more.scrollIntoView({ block: 'start' }); }
    }
  ...
  bindRadioGroup('genderSeg', 'data-value', v => { state.selected.gender = v; });
  ...
  VITAL_FIELDS.forEach(([inputId, chipId]) => {
    on($(inputId), 'input', () => { paintVital(inputId, chipId); refreshNeeds(); });
  });

  function repaintAllVitals() { VITAL_FIELDS.forEach(([i, c]) => paintVital(i, c)); }
  ...
  on($('addPhotoBtn'), 'click', () => { const i = $('photoInput'); if (i) i.click(); });
  on($('photoInput'), 'change', async e => {
    const files = Array.prototype.slice.call(e.target.files || []);
    for (const file of files) {
      if (state.images.length >= MAX_IMAGES) break;
      try { await addImage(await compressImage(file, 1280)); } catch (_) { toast('That photo could not be read'); }
    }
    e.target.value = '';
  });
  ...
  on($('backBtn'), 'click', () => { $('errorOverlay').hidden = true; });
  on($('retryBtn'), 'click', () => { $('errorOverlay').hidden = true; submitLoop(); });
  on($('newRequestBtn'), 'click', () => { $('successOverlay').hidden = true; resetForNewCase(); showView('new'); });
  on($('retryConnBtn'), 'click', () => { $('demoBanner').hidden = true; loadCaseTypes(); });
  on($('outboxPill'), 'click', () => { $('outboxOverlay').hidden = false; });
  on($('tryFlushBtn'), 'click', () => flushOutbox());
  on($('discardQueuedBtn'), 'click', () => {
    outbox.clear(); renderOutbox(); $('outboxOverlay').hidden = true;
  });
  ...
  on($('editCancelBtn'), 'click', () => { $('editOverlay').hidden = true; });
  ...
  on($('editSaveBtn'), 'click', () => {
    ...
    if (state.demo) {
      ...
      setTimeout(() => { $('editOverlay').hidden = true; }, 700);
      return;
    }
    if (!socket || !socket.connected) {
  ...
  function renderHome() {
    ...
    if (!active) {
      ...
      if (arrivedBtn) { arrivedBtn.disabled = active.status !== 'ACCEPTED'; arrivedBtn.hidden = arrived; }
      const banner = $('arrivedBanner');
  ...
  function resetForNewCase() {
    ...
    $$('#quickCase .chip, #bloodChips .chip, #ageChips .chip, #etaChips .chip, #onsetChips .chip')
