...
| Seeded accounts | none — `schema.sql` seeds no users | create real ones via `POST /api/v1/auth/register` |
...
seeds four hospitals and two ambulances.

> **No user accounts are seeded.** Earlier revisions of this guide described
> four accounts with the password `admin123`; those seeds were removed and this
> guide was wrong to keep mentioning them. Create the accounts you need — see
> [§3 step 7](#step-7--close-the-doors).
...
```json
{ "success": true, "message": "GoldenHour backend", "env": "development", "store": "memory", "store_healthy": true, "desk_auth": "ip", "hackathon_mode": true, ... }
```
...
```bash
npm test             # from the repo root: 396 checks
```

---

## 2a · The server checks its own configuration

With `NODE_ENV=production` the backend audits its configuration at boot and
**refuses to start** if anything would be unsafe in a real deployment. It
prints exactly what to change and exits `78`.

```
❌ GoldenHour refuses to start with this configuration:
   1. JWT_SECRET is not set. Generate one with: node -e "…"
   2. CORS_ORIGIN is not set, so every origin on the internet may call this API.
   3. DB_DRIVER=auto in production. "auto" falls back to an in-memory store …
   4. HACKATHON_MODE is on in production: every broadcast is sent to the two
      hardcoded demo hospitals instead of the real ones.
```

Outside production the same findings are printed as warnings and the server
starts, so the LAN demo keeps its conveniences. Nothing below can therefore be
forgotten silently — but you do still have to do it.
...
**Create real accounts.** `schema.sql` seeds no users at all, deliberately: a
published default credential in an emergency system is a disqualifying finding,
and the safest seed is none.

Note that the sign-in endpoints are **not currently mounted** — nothing in
either front-end has a login screen yet, so an exposed endpoint that mints
ER-desk tokens would be attack surface with no user. `backend/src/server.js`
carries the one line that re-enables them. Until then, insert user rows with a
hash you generate:
