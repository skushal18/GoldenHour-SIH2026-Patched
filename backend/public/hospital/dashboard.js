...
(function() {
  ...
  (function() {
    ...
    const override = cfg.FORCE_HOSPITAL || params.get("hospital") || void 0;
    cfg.ENABLE_SOUND;
    const ARRIVED_DWELL_MS = Number(cfg.ARRIVED_DWELL_MS) || 25e3;
    function apiBase() {
    ...
    function bandHr(hr) {
      ...
      if (hr > 120 || hr < 50) return "caution";
      return "good";
    }
    function bandRr(rr) {
      if (rr == null) return null;
      if (rr > 30 || rr < 8) return "critical";
      if (rr > 24 || rr < 10) return "caution";
      return "good";
    }
    function bandSpo2(v) {
    ...
    $("#soundToggle").addEventListener("click", () => {
      ...
      try {
        localStorage.setItem("goldenhour.deskSound", JSON.stringify(cfg.ENABLE_SOUND));
      } catch (_) {
      }
    });
    ...
    function beep() {
      ...
      try {
        ...
        gain.gain.setValueAtTime(1e-4, state.audio.currentTime);
        gain.gain.exponentialRampToValueAtTime(0.18, state.audio.currentTime + 0.01);
        gain.gain.exponentialRampToValueAtTime(1e-4, state.audio.currentTime + 0.35);
    ...
    async function init() {
      try {
        ...
        if (body.capacity) {
          state.capacity = Object.assign({}, state.capacity, body.capacity);
          paintCapacity();
        }
      } catch (err) {
    ...
    function startRealtime() {
      if (!window.io) return;
      const sock = state.socket = window.io({
      ...
      sock.on("hospital:identity", (d) => {
        ...
        if (!state.capacityPublished) {
          state.capacityPublished = true;
          socketSendCapacity();
        }
      });
      ...
      sock.on("capacity:changed", (payload) => {
        ...
        if (isMine) {
          state.capacity = Object.assign({}, state.capacity, payload.capacity);
          paintCapacity();
        }
      });
      sock.on("hospital:rejected", (d) => {
        alert(d.message || "Identity rejected");
      });
    }
    function paintBoard() {
    ...
    function myHospitalId() {
      return state.identity ? state.identity.hospital.hospital_id : null;
    }
    function addOrUpdatePending(card) {
      ...
      if (card.accepted_hospital_id && card.accepted_hospital_id === myHospitalId()) {
        const idx2 = state.active.findIndex((c) => c.case_code === card.case_code);
        card._accept_state = "won";
        if (idx2 < 0) state.active.push(card);
        else state.active[idx2] = card;
        paintBoard();
        return;
      }
      const idx = state.pending.findIndex((c) => c.case_code === card.case_code);
      if (idx < 0) {
        state.pending.unshift(card);
        beep();
    ...
    function removeClaimed(code, data) {
      ...
      if (data && data.accepted_hospital_id === myHospitalId()) {
        fetch(apiBase() + "/desk/queue" + (override ? "?hospital=" + override : "")).then((r) => r.json()).then((b) => {
          state.active = b.active || [];
          paintBoard();
        });
      }
      paintBoard();
    ...
    function wrapRow(cls, html) {
      return html ? `<div class="${cls}">${html}</div>` : "";
    }
    function renderPending(card) {
      var _a;
      const pat = card.patient || {};
      const v = pat.vitals || {};
      const unmet = unmetNeeds(card);
      const diverting = card.capacity && card.capacity.diversion_active;
      const css = `case priority-${card.priority || "GREEN"} ${((_a = card._accept_state) == null ? void 0 : _a.startsWith("under_warning")) ? "case-warning" : ""}`;
    ...
    function acceptTime(card) {
      ...
      const ms = /* @__PURE__ */ new Date() - new Date(card.accepted_at);
      return Math.round(ms / 1e3) + "s ago";
    }
    function renderCountdown(expires_at) {
      if (!expires_at) return "";
      function tick() {
        const ms = new Date(expires_at) - /* @__PURE__ */ new Date();
    ...
    function renderFacts(card) {
      ...
      const v = p.vitals || {};
      const f = [];
      if (p.age != null || p.gender) {
        const sex = { M: "male", F: "female", O: "other", U: "sex unknown" }[p.gender || "U"];
        f.push({ k: "PATIENT", l: ageBandLabel(p.age) + " · " + sex });
      }
    ...
    function renderShots(card) {
      ...
      return `<div class="shots-row">${images.map((src, i) => `
      <button type="button" class="shot" aria-label="Open photo ${i + 1} of ${images.length}">
        <img src="${escapeHtml(src)}" alt="Photograph ${i + 1} attached by the crew">
      </button>`).join("")}</div>`;
    }
    ...
    function paintNetwork(body) {
      state.network.filter((n) => state.identity && n.hospital_id !== state.identity.hospital.hospital_id);
    ...
    function capAge(updatedAt) {
      ...
      const mins = (Date.now() - new Date(updatedAt).getTime()) / 6e4;
      if (!Number.isFinite(mins)) return "not reported";
      if (mins > 60) return "stale · " + ageOf(updatedAt);
      return ageOf(updatedAt);
    }
    function cityName(id) {
      return { 1: "City Emergency Hospital", 2: "Apollo Hospital", 3: "Manipal Hospital", 4: "Victoria Hospital" }[id] || "Hospital #" + id;
    }
    function paintCapacity() {
      ...
      const u = state.capacity.updated_at;
      if (u) $("#capUpdated").textContent = "updated " + ageOf(u);
    }
    function setToggle(key, on) {
      const b = document.querySelector('[data-toggle="' + key + '"]');
      if (!b) return;
      b.classList.toggle("is-on", !!on);
      b.classList.toggle("is-warn", key === "diversion" && !!on);
    }
    function ageOf(iso) {
    ...
    document.querySelectorAll("[data-toggle]").forEach((b) => b.addEventListener("click", () => {
      ...
      if (t === "diversion" && state.capacity.diversion_active && !confirm("Switch this desk to diversion?")) {
        state.capacity.diversion_active = false;
      }
      paintCapacity();
      socketSendCapacity();
    }));
      ...
      capDebounce = setTimeout(() => {
        if (!state.socket || !state.socket.connected) {
          fetch(apiBase() + "/desk/capacity" + (override ? "?hospital=" + override : ""), {
            ...
            if (b && b.capacity) {
              state.capacity = Object.assign({}, state.capacity, b.capacity);
              paintCapacity();
            }
        ...
        state.socket.emit("capacity:updated", state.capacity, (ack) => {
          ...
          const _a = ack, { success, reason, message } = _a, capacity = __objRest(_a, ["success", "reason", "message"]);
    ...
    function bindCardActions() {
      document.querySelectorAll('[data-action="accept"]').forEach((btn) => btn.addEventListener("click", () => {
        ...
        fetch(apiBase() + "/desk/accept/" + code + (override ? "?hospital=" + override : ""), { method: "POST" }).then((r) => r.ok ? r.json() : r.json().then((j) => Promise.reject(j))).then(() => refreshQueue()).catch((j) => {
          toast("Accept refused: " + (j && j.message || "server"));
          refreshQueue();
        });
      }));
      document.querySelectorAll('[data-action="decline"]').forEach((btn) => btn.addEventListener("click", () => {
        const code = btn.dataset.cc;
        fetch(apiBase() + "/desk/decline/" + code + (override ? "?hospital=" + override : ""), { method: "POST" }).then(() => refreshQueue());
      }));
      ...
      document.querySelectorAll('[data-action="arrived"]').forEach((btn) => btn.addEventListener("click", () => {
        ...
        fetch(apiBase() + "/desk/arrived/" + encodeURIComponent(code) + (override ? "?hospital=" + override : ""), {
          ...
          paintArrived({ case_code: code, arrived_at: body && body.arrived_at || (/* @__PURE__ */ new Date()).toISOString() });
        }).catch(() => {
    ...
    async function printHandover(caseCode) {
      var _a, _b;
      const d = await fetchHandover(caseCode);
      if (!d) {
        toast("No handover data");
        return;
      }
      const el = $("#handover");
      el.hidden = false;
      el.querySelector(".h-meta").innerHTML = `
      <p><b>${escapeHtml(d.case_code)}</b> · ${escapeHtml(d.priority || "")} · ${escapeHtml(((_a = d.hospital) == null ? void 0 : _a.name) || "")}</p>
      ...
      el.querySelector(".h-vitals").innerHTML = (d.vitals_initial_vs_latest || []).map((v) => {
        var _a2, _b2;
        return `<div>${escapeHtml(v.key)}: ${(_a2 = v.initial) != null ? _a2 : "—"} → ${(_b2 = v.latest) != null ? _b2 : "—"}</div>`;
      }).join("");
      ...
      el.querySelector(".h-footer").innerHTML = `<hr><p>Generated by GoldenHour v5 · ${escapeHtml(((_b = d.hospital) == null ? void 0 : _b.name) || "")} · ${(/* @__PURE__ */ new Date()).toISOString()} · case ${escapeHtml(d.case_code)}</p>`;
    ...
    document.body.addEventListener("click", (e) => {
      const shot = e.target.closest && e.target.closest(".shots-row .shot, .shots-row img");
      if (!shot) return;
      const img = shot.tagName === "IMG" ? shot : shot.querySelector("img");
      if (!img) return;
      $("#lightboxImg").src = img.src;
      $("#lightboxImg").alt = img.alt || "Photograph attached to this case";
    ...
    async function openAnalytics() {
      ...
      try {
        ...
        body.innerHTML = tiles.map(([label, value]) => `<div class="stat-tile"><span class="eyebrow">${escapeHtml(label)}</span><b>${escapeHtml(String(value))}</b></div>`).join("");
        $("#analyticsStore").textContent = d.store === "memory" ? "In-memory store — these figures reset every time the server restarts." : "Persisted in " + (d.store || "the database") + ".";
      } catch (_) {
    ...
    if (analyticsClose) analyticsClose.addEventListener("click", () => {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      ["#analyticsPanel", "#lightbox"].forEach((sel) => {
        const el = $(sel);
        if (el) el.hidden = true;
      });
    });
