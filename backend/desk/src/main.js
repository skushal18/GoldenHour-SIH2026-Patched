...
(function(){
  ...
  /* === theme ===
     There is no colour-theme control here or in the ambulance app: GoldenHour
     has one visual identity, and a charge nurse mid-shift should not be
     choosing palettes. The stylesheet still answers the device's own
     light/dark preference, so a night shift gets the dark surfaces with
  ...
  function paintArrived(data){
    ...
    /* Reached-hospital can arrive twice — once from the button's own
       response and once from the socket. The second must be a no-op, not a
       second dwell timer that clears the card early. */
  ...
  function renderActive(card){
    const pat = card.patient || {}; const v = pat.vitals || {};
    /* NOT `outcome-arrived` — that class is the ribbon's, and putting it on
       the article gave the whole card a green background and green text,
       which flattened the acrylic and left the case title barely legible
       against it. The card gets its own state class. */
  ...
  function renderFacts(card){
    ...
    const v = p.vitals || {};
    const f = [];
    /* The age band, not the representative number behind it. The crew chose
       "Adult"; printing 35 to a doctor would invent a precision nobody
       measured. Same mapping as the ambulance app — see shared/age-bands.js. */
  ...
  /* Scene, injury, ECG strip. These come down on every card in
     `card.images` and the lightbox below has always listened for them, but no
     renderer ever emitted the markup — so four photographs a crew stopped to
     take were being dropped on the floor at the one place they matter.

     A <button> wrapper rather than a bare <img>: it makes each thumbnail
     reachable and openable from the keyboard, which a desk operator with one
  ...
  function bindCardActions(){
    ...
    /* The crew records arrival from the app. When they cannot — the phone is
       flat, the socket dropped, the patient went straight through the doors —
       the desk needs the same capability, and this uses exactly the same
       endpoint and the same case lifecycle. Nothing new is invented here. */
    document.querySelectorAll('[data-action="arrived"]').forEach(btn => btn.addEventListener('click', () => {
      ...
      fetch(apiBase() + '/requests/' + encodeURIComponent(code) + '/arrived', {
        ...
        .then(({ ok, body }) => {
          ...
          /* paintArrived also runs from the case:arrived socket event, so the
             board is right whichever arrives first — and it is idempotent. */
          paintArrived({ case_code: code, arrived_at: (body && body.arrived_at) || new Date().toISOString() });
        })
        .catch(() => {
  ...
  document.body.addEventListener('click', e => {
    /* A mouse lands on the <img>; a keyboard Enter fires on the <button>. */
  ...
  /* One handler for every X on the board. */
