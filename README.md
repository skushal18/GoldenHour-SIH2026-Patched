...
shared/
  age-bands.js             Baby · Child · Adult · Senior · Unknown, shared
  design/
    tokens.css             One palette, both front-ends
    primitives.css         Including the shared acrylic surface
...
### Age is a band, not a number

A crew at a roadside rarely knows a patient's age and should not have to
invent one to get past a form. Age is chosen as **Baby · Child · Adult ·
Senior citizen · Unknown**.

The wire format is unchanged — `age` is still the single number the backend,
the validator, triage and the ER board already expect. Each band carries a
representative value that travels, and `describeAgeBand()` maps any number
back to its band, so both front-ends display the band. That matters: showing a
selected "Adult" to a doctor as the bare number 35 would present a precision
nobody measured. The mapping lives in `shared/age-bands.js` so the two
front-ends can never disagree about what "Child" means.

Age and sex are marked Required and default to Unknown — a real answer, one
tap away. Neither is a hard gate: a demographic field must never stand between
a crew and a hospital.
...
- The acrylic surfaces declare their opaque form first and layer translucency
  inside `@supports`, so a WebView without `backdrop-filter` gets a solid card
  rather than a grey smear over clinical text.
- Light and dark follow the device's own preference. There is no colour-theme
  control: GoldenHour has one visual identity, and a crew mid-shift should not
  be choosing palettes.
